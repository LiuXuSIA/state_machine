...
void takeOffCommand_cb(const state_machine::requestCommand_L2F::ConstPtr& msg)
{
    ...
    {
        takeOffCommand = *msg;
        ROS_INFO("uav3 received the take off command!!");
...
void communication_test_cb(const state_machine::requestCommand_L2F::ConstPtr& msg)
{
    ...
    ROS_INFO("uav3 receive the communication request.");
}
...
{
    ros::init(argc, argv, "offboard_node_uav_3");
    ros::NodeHandle nh;

    takeOffStatus.UAV_index = 3;
    takeOffStatus.value = 1;

    communicationStatus.UAV_index = 3;
