/*************************************************************************
@file           state_machine_Full_Vision_v5.cpp
@date           2018/10/20
@author         liuxu
@email          liuxu.ccc@gmail.com
@description    takeoff mode;min distance from ground;place glue position;
...
#define MOVE_DISTANCE_SINGLE    1.5
...
geometry_msgs::PoseStamped position_place_glue;
    ...
    switch(current_mission_state)
    {
        ...
        case position_construction_go:
        {
                                ...
                                current_position.pose.position.z,position_construction.pose.position.z) < LOCATE_ACCURACY_HIGH)
            {
                ...
                if(mission_loop == -1)
                {
                    position_place.pose.position.z = -fix_target_position.construction_z + PLACE_GLUE_HEIGHT;
                }
                else
                {
        ...
        case component_place:
        {
            ...
            if(mission_loop == -1)
            {
                ...
                {
                    position_place_glue.pose.position.x = position_construction.pose.position.x + 0.5 * MOVE_DISTANCE_SINGLE * cos(yaw_sp);
                    position_place_glue.pose.position.y = position_construction.pose.position.y + 0.5 * MOVE_DISTANCE_SINGLE * sin(yaw_sp);
                    position_place_glue.pose.position.z = -fix_target_position.construction_z + PLACE_GLUE_HEIGHT;
                    if (dis_enable2 == true)
                    {
                        ROS_INFO("move route 1");
                        dis_enable2 = false;
                    }
                }
                else if (ros::Time::now() - mission_last_time < ros::Duration(GLUE_TIME_SINGLE * 3))
                {
                    ...
                    {
                        ROS_INFO("move route 2");
                        dis_enable2 = true;
                    }
                }
                else if (ros::Time::now() - mission_last_time < ros::Duration(GLUE_TIME_SINGLE * 4))
                {
                    position_place_glue.pose.position.x = position_construction.pose.position.x;
                    position_place_glue.pose.position.y = position_construction.pose.position.y;
                    position_place_glue.pose.position.z = -fix_target_position.construction_z + PLACE_GLUE_HEIGHT;
                    if (dis_enable2 == true)
                    {
                        ROS_INFO("move route 3");
                        dis_enable2 = false;
                    }
                }
                else
                ...
                local_pos_pub.publish(position_place_glue);
            }
            else
            {
                pose_pub = position_place;
